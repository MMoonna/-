...
//引入接口
import { requLogin, reqUserInfo, reqLogout } from '@/api/user/index'
// 引入接口类型
import type {
  LoginForm,
  loginResponseData,
...
export const useUserStore = defineStore('user', {
  ...
  actions: {
    ...
    async userlogin(data: LoginForm) {
      let result: loginResponseData = await requLogin(data)
      // 因为要根据是否登录成功进行不同的活动，所以需要返回一个值区别是登陆成功还是登陆失败
      if (result.code == 200) {
        //pinia仓库存储一下token
        //由于pinia|vuex存储数据其实利用js对象
        this.token = result.data as string //将token存储到仓库中
        return 'ok'
      } else {
        // 抛出错误返回
    ...
    async getUserInfo() {
      ...
      let userInfo: userInfoReponseData = await reqUserInfo()
